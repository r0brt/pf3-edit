...
pub(super) fn find_first_in_bounds_from(
    text: &str,
    pattern: &str,
    bounds: Option<(usize, usize)>,
    from_col: usize,
    ...
    let search_start_char = from_col.max(start_char);
...
pub(super) fn find_first_in_bounds_before(
    text: &str,
    pattern: &str,
    bounds: Option<(usize, usize)>,
    before_col: usize,
    ...
    let (start_char, end_char) = bounded_char_range(text, bounds);
    let search_end_char = before_col.min(end_char);
    if search_end_char <= start_char {
        return None;
    }

    let start_byte = char_to_byte_index(text, start_char);
    let end_byte = char_to_byte_index(text, search_end_char);
